...
contract("Averageprice", (accounts) => {
  it("The end of the auction transfers the correct amount of money", async () => {
    ...
    let averageprice = await Averageprice.new(accounts[0], auctionItem.item, {
      from: accounts[0],
    });

    await averageprice.bid(blindValues[0], accounts[1], "pubkey", {
      from: accounts[1],
      value: bidValues[0],
    });
    await averageprice.bid(blindValues[1], accounts[2], "pubkey", {
      from: accounts[2],
      value: bidValues[1],
    });

    await averageprice.bid(blindValues[2], accounts[3], "pubkey", {
      from: accounts[3],
      value: bidValues[2],
    });
